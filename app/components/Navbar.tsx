...
const Navbar = () => {
  ...
  // URUTAN PENTING: Produk & Portofolio diletakkan sebelum Layanan & Harga.
  // Twenti adalah app studio yang membangun produk sendiri lebih dulu;
  // layanan adalah lini sekunder. Urutan navbar harus mencerminkan itu,
  // bukan menempatkan jasa sebagai identitas utama.
  const navLinks = [
    { key: "nav.home", href: "/" },
    { key: "nav.products", href: "/produk" },
    { key: "nav.portfolio", href: "/portofolio" },
    { key: "nav.services", href: "/layanan" },
    { key: "nav.pricing", href: "/paket-harga" },
    { key: "nav.about", href: "/tentang" },
    { key: "nav.insight", href: "/insight" },
  ...
  return (
    <>
      ...
      >
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex items-center justify-between h-16">
            ...
            {/* Desktop Navigation */}
            <div className="hidden lg:flex items-center gap-0.5 bg-dark-800/50 rounded-full p-1">
              {navLinks.map((link, index) => (
                <Link
                  key={link.href}
                  href={link.href}
                  style={{ animationDelay: `${index * 45}ms` }}
                  className={`relative text-[13px] font-medium transition-all duration-300 px-3 py-2 rounded-full whitespace-nowrap animate-[navItemIn_0.4s_ease-out_backwards] ${
                    isActive(link.href)
                      ? "text-white bg-orange-500/20 border border-orange-500/30"
                      : "text-gray-400 hover:text-white hover:bg-white/10 hover:-translate-y-0.5"
                  }`}
                >
                  {t(link.key)}
                </Link>
      ...
      >
        ...
        >
          <div className="bg-dark-800/50 backdrop-blur-md rounded-2xl p-6 border border-white/10 mx-4 max-h-[85vh] overflow-y-auto">
            {navLinks.map((link, index) => (
              <Link
                key={link.href}
                href={link.href}
                ...
                style={{
                  animationDelay: isMobileMenuOpen ? `${index * 40}ms` : "0ms",
                }}
                className={`block text-lg font-semibold py-2.5 px-6 rounded-lg transition-all duration-300 ${
                  isMobileMenuOpen
